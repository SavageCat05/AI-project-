...
def run_experiments(dimension=10, max_evals=60000):
    ...
    pd.DataFrame(fitness_scores_data).to_csv('CEC2022_benchmarks_results/one_run_fitness_scores_CEC2022.csv', index=False)
...
def plot_fitness_scores(fitness_data):
    ...
    plt.xlabel("Benchmark Functions (CEC2022)")
    plt.ylabel("Fitness Value")
    plt.title("Single-Run Fitness Comparison on 10 CEC2022 Functions")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()

    os.makedirs('CEC2022_benchmarks_results', exist_ok=True)
    plt.savefig('CEC2022_benchmarks_results/one_run_fitness_scores_curve_CEC2022.png')
    # plt.show()
