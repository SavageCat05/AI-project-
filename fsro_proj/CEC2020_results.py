...
def run_experiments(dimension=10, max_evals=60000, num_runs=10):
    ...
    pd.DataFrame(results).to_csv('CEC2020_benchmarks_results/comparison_result_CEC2020.csv', index=False)
    np.save('CEC2020_benchmarks_results/convergence_data.npy', convergence_data)
...
def plot_comparison(convergence_data, functions_to_plot=None):
    ...
    for func_name in functions_to_plot:
        ...
        os.makedirs('CEC2020_benchmarks_results', exist_ok=True)
        plt.savefig(f"CEC2020_benchmarks_results/{func_name}_convergence.png")
