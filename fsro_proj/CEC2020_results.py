...
def run_experiments(dimension=10, max_evals=60000):
    ...
    os.makedirs("CEC2020_benchmarks_results", exist_ok=True)
    pd.DataFrame(fitness_scores_data).to_csv('CEC2020_benchmarks_results/one_run_fitness_scores_CEC2020.csv', index=False)
...
def plot_fitness_scores(fitness_data):
    ...
    plt.xlabel("Benchmark Functions (CEC2020)")
    plt.ylabel("Fitness Value")
    plt.title("Single-Run Fitness Comparison on 10 CEC2020 Functions")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()

    os.makedirs('CEC2020_benchmarks_results', exist_ok=True)
    plt.savefig('CEC2020_benchmarks_results/one_run_fitness_scores_curve_CEC2020.png')
    # plt.show()
